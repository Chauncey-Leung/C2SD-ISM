function [direct_lattice_vectors,corrected_basis_vectors, vec_estimate_manual] = getBasicVec(imgStack,param,isPreBasis,isDisplay)
%   This function estimates the reciprocal lattice basis vectors based on
%   the Fourier spectra of a 3D image stack. It refines initial guesses 
%   (manually selected or provided) using peak localization and least-squares
%   fitting, and then converts the Fourier basis vectors into direct-space
%   lattice vectors.
%
%   Inputs:
%   -------
%   imgStack         : 3D image stack (Height × Width × Frames)
%   param            : Structure with parameters:
%                      - range: search radius in Fourier domain
%                      - extent: local window size for peak detection
%                      - reference: index or 'Sum' for computing reference spectrum
%                      - vec_estimate_manual: 2×2 initial guess of lattice vectors
%   isPreBasis       : If true, use `param.vec_estimate_manual` directly.
%                      If false, user selects vectors manually via GUI.
%   isDisplay        : If true, shows a visualization of the final vectors.
%
%   Outputs:
%   --------
%   direct_lattice_vectors   : 2×2 direct-space lattice basis matrix
%   corrected_basis_vectors  : 2×2 refined basis vectors in Fourier domain
%   vec_estimate_manual      : The initial (or manually selected) basis vectors
%
...
Img = imgStack;
[Height, Width, Counts] = size(imgStack);
